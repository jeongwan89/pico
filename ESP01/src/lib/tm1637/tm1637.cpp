...
int TM1637::display_float(float value, uint8_t decimal_point) {
    ...
    uint8_t segs[4];
    int temp_value = int_value;
    // 각 자리수 추출 (앞자리 0은 BLANK로 치환)
    for (int i = 3; i >= 0; --i) {
        segs[i] = encodeDigit(temp_value % 10);
        temp_value /= 10;
    }
    // 앞자리 0을 BLANK로 치환 (음수면 부호는 남김)
    int first_digit = negative ? 1 : 0;
    for (int i = first_digit; i < 4 - decimal_point; ++i) {
        if (segs[i] == encodeDigit(0)) {
            segs[i] = BLANK_SEGMENT;
        } else {
            break;
        }
    }
